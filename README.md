...
## Входные данные
В качестве входных данных приложения используются строки с названиями (либо с набором параметров как на русском, так и на английском языке) компонентов и их количеством. Перед наименованием можно добавить необязательное позиционное обозначение. Столбцы разделяются точкой с запятой либо табуляцией.
...
Поддерживаются распространённые обозначения пассивных компонентов (`R`, `C`, `L`, `FB`), микросхем (`DA`, `DD`, `DS`, `U`, `IC`), кварцев (`Y`, `ZQ`), реле (`K`, `KA`, `KM`, `KT`, `KV`), диодов и транзисторов (`D`, `VD`, `Q`, `VT`, `VS`), а также светодиодов, предохранителей, трансформаторов, разъёмов, переключателей и ряда электромеханических компонентов. Таблица соответствий находится в `Components/ReferenceDesignator.py` и может расширяться для обозначений конкретной САПР или предприятия.
