...
## Входные данные
В качестве входных данных приложения используются строки с названиями (либо с набором параметров как на русском, так и на английском языке) компонентов и их количеством. Перед наименованием можно добавить необязательное позиционное обозначение или явно указать тип компонента. Столбцы разделяются точкой с запятой либо табуляцией.

Позиционное обозначение может содержать один элемент (`DD1`), перечень (`DD1,DD2`), диапазон (`DD1...DD5`) или их комбинацию (`DD1,DD2,DD3...DD7`). Если оно указано, тип компонента определяется по нему; иначе используется прежнее распознавание по наименованию.

Вместо позиционного обозначения в первой колонке можно указать отображаемый приложением тип, например `Конденсатор`, `Резистор`, `Микросхема` или `Диод`. Регистр букв не учитывается. Явный тип имеет приоритет над распознаванием по наименованию. Например: `Микросхема;STM32H743ZIT6;2`.

Основная таблица соответствий повторяет обозначения из шаблона Altium Designer: в частности, `D`/`DA`/`DD` обозначают микросхемы, `VD` — диод, `F` — разрядник, `FU` — предохранитель, `U` — оптопару, а `Z`/`ZQ` — кварцевый резонатор. Также поддерживаются дополнительные распространённые обозначения, не конфликтующие с шаблоном. Полная таблица находится в `Components/ReferenceDesignator.py` и может расширяться для обозначений конкретной САПР или предприятия.
